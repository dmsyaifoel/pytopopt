'''
Inaccurate but very fast TOPOPT implementation (minimal compliance) using an adaptive gradient descent method
(it's in fact an adaption of Newton's method, adapted to rootfinding of a function R^n->R, so it doesn't actually converge to a minimum
Instead it approaches a target compliance (ctarget) and volume fraction (volfrac)
'''
import numpy as np
...
x = topopt(nx, ny, 60, .3, fixeddofs, [[1, -1]], 50)
